...
Two independent things can be "out of date," each handled differently:

- **A game/port you've installed** — checked in the background (throttled to once every 12 hours, to stay under GitHub/GitLab's unauthenticated API rate limit) against its repo's latest release. A newer tag *or* a newer publish/asset date flags it — the date check exists specifically for projects that never bump their tag and just keep replacing the same "latest" release's file. Shows up as an **Update** button next to **Play**.
- **Ports Launcher itself** — same tag/date check, run against this repo's own releases. When one's out, the footer's **GitHub** button becomes **Update**; the link is unchanged, so it's still on you to grab and install the new build.
