...
/// `ports.json` du dépôt AU MOMENT DE LA COMPILATION, embarqué dans l'exe --
/// voir `main()`, qui l'écrit sur disque si aucun `ports.json` local
/// n'existe : le premier lancement reste amorçable hors-ligne sans fichier
/// fourni à part, et `catalog_sync` rafraîchit ensuite le catalogue en tâche
/// de fond.
pub fn embedded_fallback() -> &'static str {
    include_str!("../../ports.json")
}

/// Fusionne le catalogue local dans le principal -- une entrée locale dont
/// `folder_name` existe déjà côté principal REMPLACE celle du principal
...
pub fn merge_local_catalog(mut main: Vec<Port>, local: Vec<Port>) -> Vec<Port> {
    for port in local {
        main.retain(|p| p.folder_name != port.folder_name);
        main.push(port);
    }
    main
}
...
mod tests {
    ...
    fn charge_des_ports_valides() {
        let path = write_temp(
            "ok",
            r#"{"ports":[{"name":"A","folder_name":"a","source":"https://example.com/a.zip"}]}"#,
        );
    ...
    fn entree_sans_name_est_ignoree_pas_fatale() {
        let path = write_temp(
            "missing_name",
            r#"{"ports":[{"folder_name":"a","source":"s"},{"name":"B","folder_name":"b","source":"s"}]}"#,
        );
        let ports = load_config(&path).unwrap();
        assert_eq!(ports.len(), 1);
    ...
    fn champ_mal_type_est_ignore_silencieusement() {
        let path = write_temp(
            "bad_type",
            r#"{"ports":[{"name":5,"folder_name":"a","source":"s"},{"name":"B","folder_name":"b","source":"s"}]}"#,
        );
        let ports = load_config(&path).unwrap();
        assert_eq!(ports.len(), 1);
        assert_eq!(ports[0].name, "B");
    ...
    fn load_local_config_accepte_une_entree_sans_source() {
        let path = write_temp("local_ok", r#"{"ports":[{"name":"My Game","folder_name":"my-game"}]}"#);
    ...
    fn merge_local_catalog_ajoute_les_nouvelles_entrees() {
        let main = vec![port_from_value(&serde_json::json!({"name": "A", "folder_name": "a", "source": "s"})).unwrap()];
        let local = vec![port_from_value(&serde_json::json!({"name": "B", "folder_name": "b"})).unwrap()];
        let merged = merge_local_catalog(main, local);
        assert_eq!(merged.len(), 2);
    }

    #[test]
    fn merge_local_catalog_une_entree_locale_remplace_le_principal_sur_le_meme_folder_name() {
        let main = vec![port_from_value(&serde_json::json!({"name": "Official", "folder_name": "a", "source": "s"})).unwrap()];
        let local = vec![port_from_value(&serde_json::json!({"name": "Customized", "folder_name": "a"})).unwrap()];
        let merged = merge_local_catalog(main, local);
