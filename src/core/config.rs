...
impl fmt::Display for ConfigError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ConfigError::NotAnObject => write!(f, "the root of ports.json is not an object"),
            ConfigError::NoPorts => write!(f, "ports.json contains no usable port"),
        }
    }
}
...
pub fn parse_catalog(text: &str) -> Result<Vec<Port>, ConfigError> {
    ...
    let ports: Vec<Port> = obj
        .get("ports")
        .and_then(serde_json::Value::as_array)
        .map(|raw| raw.iter().filter_map(|p| port_from_value(p).ok()).collect())
        .unwrap_or_default();
    if ports.is_empty() {
...
pub fn load_local_config(path: &Path) -> Vec<Port> {
    ...
    let Some(raw_ports) = data.get("ports").and_then(serde_json::Value::as_array) else { return Vec::new() };
    raw_ports
        .iter()
...
mod tests {
    ...
    fn charge_des_ports_valides() {
        let path = write_temp("ok", r#"{"ports":[{"name":"A","folder":"a","source":"https://example.com/a.zip"}]}"#);
    ...
    fn entree_sans_name_est_ignoree_pas_fatale() {
        let path = write_temp("missing_name", r#"{"ports":[{"folder":"a","source":"s"},{"name":"B","folder":"b","source":"s"}]}"#);
        let ports = load_config(&path).unwrap();
        assert_eq!(ports.len(), 1);
    ...
    fn champ_mal_type_est_ignore_silencieusement() {
        let path = write_temp("bad_type", r#"{"ports":[{"name":5,"folder":"a","source":"s"},{"name":"B","folder":"b","source":"s"}]}"#);
        let ports = load_config(&path).unwrap();
        assert_eq!(ports.len(), 1);
        assert_eq!(ports[0].name, "B");
        let _ = fs::remove_file(&path);
    ...
    #[test]
    fn load_local_config_fichier_absent_est_vide() {
        let mut path = std::env::temp_dir();
    ...
    #[test]
    fn load_local_config_json_mal_forme_est_vide() {
    ...
    fn merge_local_catalog_ajoute_les_nouvelles_entrees() {
        ...
        assert_eq!(merge_local_catalog(main, local).len(), 2);
    }
