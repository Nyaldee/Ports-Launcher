...
pub struct Port {
    ...
    /// `{"windows": ..., "linux": ...}` -- voir `platform_utils::resolve_per_platform`.
    pub source: Value,
    ...
    pub mods_url: Option<String>,
    pub image_url: Option<String>,
    pub save_folder: Option<Value>,
    ...
    /// dossier de save (voir `installer::backup_local_save`/
    /// `restore_backed_up_save`, sauvegardés dans des sous-dossiers
    /// SÉPARÉS de `.saves_backup` pour ne jamais écraser l'un avec l'autre
    /// même si les deux contiennent des fichiers de même nom).
    pub save_folder2: Option<Value>,
    pub source_type: SourceType,
    pub repo: Option<String>,
    /// `true` si le `.exe` de ce port est en réalité un conteneur zip à
    /// décompresser (voir `installer::extract`), jamais déduit du contenu --
    /// un exécutable portable légitime peut techniquement s'ouvrir comme un
    /// zip sans en être un. Défaut `false`.
    pub exe_is_archive: bool,
    /// Sous-chaîne du nom de l'asset à télécharger en priorité (voir
    /// `platform_utils::resolve_preferred_asset`, `asset_select::pick_asset`)
    ...
    /// `executable`/`save_folder` (`{"windows": ..., "linux": ...}` ou une
    /// simple chaîne). `None` -> heuristique standard, inchangée.
    pub preferred_asset: Option<Value>,
    /// Nom (sous-chaîne) de l'exécutable réel dans un `.exe` traité comme
    /// une archive (voir `Port::exe_is_archive`, `installer::find_exe_folder`)
    /// -- écarte les exécutables annexes (prérequis, outils) du dossier
    /// gardé. `None` -> comportement par défaut, sans filtre par nom.
    pub base_exe: Option<String>,
}
...
impl Port {
    ...
    pub fn key(&self) -> &str {
        self.repo.as_deref().unwrap_or(&self.folder_name)
    }
...
/// `ports.local.json`. `name`/`folder_name` requis et strictement typés
...
pub fn port_from_value(d: &Value) -> Result<Port, PortParseError> {
    ...
    let folder_name = obj
        .get("folder_name")
        .and_then(Value::as_str)
        .ok_or_else(|| PortParseError("\"folder_name\" missing or invalid".to_string()))?
        .to_string();
    ...
    let mods_url = obj.get("mods_url").and_then(Value::as_str).map(str::to_string);
    let image_url = obj.get("image_url").and_then(Value::as_str).map(str::to_string);
    let executable = obj.get("executable").cloned();
    let save_folder = obj.get("save_folder").cloned();
    let save_folder2 = obj.get("save_folder2").cloned();
    let exe_is_archive = obj.get("exe_is_archive").and_then(Value::as_bool).unwrap_or(false);
    let preferred_asset = obj.get("preferred_asset").cloned();
    let base_exe = obj.get("base_exe").and_then(Value::as_str).map(str::to_string);
...
mod tests {
    ...
    fn from_value_rejette_name_manquant_ou_invalide() {
        assert!(port_from_value(&json!({"folder_name": "f", "source": "s"})).is_err());
        assert!(port_from_value(&json!({"name": 5, "folder_name": "f", "source": "s"})).is_err());
    }

    #[test]
    fn from_value_rejette_folder_name_manquant() {
        assert!(port_from_value(&json!({"name": "n", "source": "s"})).is_err());
    }

    #[test]
    fn from_value_source_manquante_est_un_port_local() {
        let p = port_from_value(&json!({"name": "n", "folder_name": "f"})).unwrap();
    ...
    fn from_value_rejette_source_mal_typee() {
        assert!(port_from_value(&json!({"name": "n", "folder_name": "f", "source": 5})).is_err());
    }

    #[test]
    fn from_value_tolere_champs_optionnels_mal_types() {
        let p = port_from_value(&json!({
            "name": "n", "folder_name": "f", "source": "s",
            "tags": "not-a-list", "website": 5, "mods_url": [], "image_url": {}
        }))
        .unwrap();
        assert!(p.tags.is_empty());
        assert_eq!(p.website, None);
        assert_eq!(p.mods_url, None);
    ...
    fn from_value_filtre_les_tags_non_chaine() {
        let p = port_from_value(&json!({
            "name": "n", "folder_name": "f", "source": "s", "tags": ["a", 5, "b", null]
        }))
        .unwrap();
    ...
    #[test]
    fn key_utilise_repo_sinon_folder_name() {
        let p = port_from_value(&json!({"name": "n", "folder_name": "f", "source": "https://github.com/o/p"})).unwrap();
        assert_eq!(p.key(), "o/p");
        let p = port_from_value(&json!({"name": "n", "folder_name": "f", "source": "https://example.com/x.zip"})).unwrap();
        assert_eq!(p.key(), "f");
    }
}
