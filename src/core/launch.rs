...
//! `executable_detect::resolve_executable`) -- toujours un binaire connu et
//! déjà vérifié existant sur disque, jamais un chemin arbitraire à faire
//! résoudre par le shell.
//!
//! Windows : `.lnk`/`.bat` ne sont pas des PE, `CreateProcess` (ce
//! qu'utilise `Command::spawn`) ne sait exécuter ni l'un ni l'autre
//! directement -- les deux passent par le Shell (voir `launch_via_shell`).
//! Linux : un port n'a que des binaires natifs (ELF) ou des scripts avec
//! shebang, tous deux directement exécutables par le noyau -- pas
//! d'équivalent de cette distinction.

use std::io;
use std::path::Path;
use std::process::{Child, Command};

/// Un `.exe`/binaire ELF lancé via `Command::spawn` donne un vrai
/// `std::process::Child` (attente/nettoyage standards). Sous Windows, un
/// `.lnk` lancé via `ShellExecuteExW` ne donne qu'un `HANDLE` Win32 brut
/// (avec `SEE_MASK_NOCLOSEPROCESS`, sinon le Shell le referme lui-même et on
/// ne peut plus jamais savoir si le jeu tourne encore) -- ce type unifie les
/// deux pour que `is_port_running` n'ait pas à connaître la différence.
...
/// `cwd` = dossier de l'exécutable, même raison que côté Windows. Le bit
/// exécutable manque souvent après extraction d'une archive qui ne
/// préservait pas les permissions Unix (zip/7z, contrairement à tar) --
/// posé ici avant de lancer plutôt que de faire échouer `Command::spawn` sur
/// un `Permission denied` : ça couvre aussi bien un binaire ELF/AppImage
/// qu'un script, contrairement à un repli sur `sh` (correct pour un script,
/// mais lancerait `sh` sur le contenu binaire d'un AppImage). `sh` ne reste
/// un dernier recours que si poser le bit échoue (filesystem en lecture
/// seule...).
#[cfg(target_os = "linux")]
pub fn launch(exe_path: &Path) -> io::Result<LaunchedProcess> {
...
/// `xdg-open` : équivalent standard freedesktop de `ShellExecuteW("open", ...)`,
/// passé en un seul argument `Command` -- jamais un shell intermédiaire à
/// injecter (même raison que côté Windows).
#[cfg(target_os = "linux")]
fn shell_execute_open(target: &str) {
