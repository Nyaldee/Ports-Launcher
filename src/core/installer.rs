...
use super::platform_utils::{resolve_per_platform, resolve_preferred_asset, resolve_save_folder, safe_join};
...
/// diffère de `dest` seulement pour un lien de partage pixeldrain
/// (voir `pixeldrain_file_id`), réécrit vers le point de téléchargement
/// direct et renommé avec la vraie extension quand elle est connue.
    ...
    if let Some(id) = pixeldrain_file_id(&url) {
        if let Some(real_name) = pixeldrain_real_name(id) {
            if let Some(parent) = dest.parent() {
                dest = parent.join(safe_download_name(&real_name));
            }
        }
...
/// Cherche récursivement tous les `.exe` sous `dir` -- voir `find_exe_folder`.
fn collect_exe_files(dir: &Path, out: &mut Vec<PathBuf>) -> io::Result<()> {
    for entry in fs::read_dir(dir)?.filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.is_dir() {
            collect_exe_files(&path, out)?;
...
/// Dossier du vrai exécutable dans un `.exe` traité comme une archive (voir
/// `Port::exe_is_archive`) -- ce dossier, pas la racine du conteneur, est
/// fusionné dans `dest_dir`.
///
/// `base_exe` -- voir `Port::base_exe` -- filtre sur ce nom précis quand
/// fourni, pour écarter des exécutables annexes (installeurs de prérequis,
/// utilitaires) plutôt que de les compter comme candidats. Sans lui,
/// plusieurs `.exe` dans des dossiers différents -> erreur plutôt qu'une
/// devinette au hasard ; dans le même dossier (jeu + updater), pas ambigu.
fn find_exe_folder(staging: &Path, base_exe: Option<&str>) -> Result<PathBuf, InstallError> {
    let mut matches = Vec::new();
    collect_exe_files(staging, &mut matches).install_err()?;

    if let Some(target) = base_exe {
        let target_lower = target.to_lowercase();
        matches.retain(|p| p.file_name().and_then(|n| n.to_str()).is_some_and(|n| n.to_lowercase().contains(&target_lower)));
        return match matches.len() {
            0 => Err(InstallError::Message(format!("No executable matches \"{target}\" (see \"base_exe\" for this port)."))),
            1 => Ok(matches[0].parent().unwrap_or(staging).to_path_buf()),
            n => Err(InstallError::Message(format!("{n} executables match \"{target}\" -- please report this port."))),
        };
    }

    if matches.is_empty() {
        return Err(InstallError::Message("Couldn't find any executable inside this installer.".to_string()));
    }
    let mut parents: Vec<PathBuf> = matches.iter().filter_map(|p| p.parent().map(Path::to_path_buf)).collect();
    parents.sort();
    parents.dedup();
    match parents.len() {
        1 => Ok(parents.into_iter().next().unwrap()),
        n => Err(InstallError::Message(format!(
            "Found executables in {n} different folders inside this installer, don't know which one to keep -- please report this port."
        ))),
    }
}
...
/// installeur NSIS), jamais deviné depuis le contenu. `base_exe` -- voir
/// `find_exe_folder`.
fn extract(
    archive: &Path,
    dest_dir: &Path,
    ...
    let is_exe_archive = name_lower.ends_with(".exe") && exe_is_archive;
    ...
    let merge_root: PathBuf = if is_known_archive {
        let kind = extract_archive_via_7z(archive, staging, MAX_UNCOMPRESSED_SIZE)?;
        if is_exe_archive {
            // Un installeur NSIS écrit déjà chaque fichier à sa vraie
            // destination (dérivée de son script) -- pas de dossier à
            // deviner, sauf si le port veut explicitement écarter le reste
            // (prérequis, outils) via `base_exe`. Un zip déguisé ou une
            // archive RAR, eux, n'ont aucune notion de destination :
            // `find_exe_folder` reste nécessaire même sans `base_exe`.
            let is_nsis = kind.eq_ignore_ascii_case("Nsis");
            if is_nsis && base_exe.is_none() {
                staging.to_path_buf()
            } else {
                find_exe_folder(staging, base_exe)?
            }
        } else {
...
pub fn install_port(
    ...
    let dest_dir = safe_join(library_dir, &port.folder_name).map_err(InstallError::Message)?;
    ...
    // Per-plateforme comme executable/save_folder (voir
    ...
    extract(&archive_path, &dest_dir, library_dir, port.exe_is_archive, port.base_exe.as_deref(), &mut on_progress)?;
    restore_backed_up_save(port.save_folder.as_ref(), library_dir, &dest_dir, &port.folder_name, "save_folder");
    restore_backed_up_save(port.save_folder2.as_ref(), library_dir, &dest_dir, &port.folder_name, "save_folder2");

    if let Some(url) = &port.image_url {
        if url.starts_with("http://") || url.starts_with("https://") {
            image_cache::cache_image(url, cache_dir, &port.folder_name);
        }
    }
...
/// Dossier de sauvegarde temporaire d'une save locale pendant qu'un port
/// est désinstallé -- HORS de `library_dir/<folder_name>` (donc jamais
/// supprimé par le `remove_dir_all` d'`uninstall_port`), mais DANS
/// `library_dir` (même raison que le dossier de transit dans `extract()` :
/// tout reste dans un dossier "Library" prévisible, jamais dans `%TEMP%`).
/// `field` -- `"save_folder"` ou `"save_folder2"` -- isole chaque champ dans
/// son PROPRE sous-dossier de `.saves_backup/<folder_name>/`, pour que les
/// deux ne s'écrasent jamais l'un l'autre même si `save_folder`/
/// `save_folder2` contiennent des fichiers de même nom (ex: tous les deux
/// nommés "slot1.dat", pour une version portable et une version "normale" du
/// même jeu).
fn save_backup_dir(library_dir: &Path, folder_name: &str, field: &str) -> PathBuf {
    library_dir.join(".saves_backup").join(folder_name).join(field)
}

/// Si `save_folder` (ou `save_folder2`, voir `field`) est configuré ET
/// pointe vers un chemin qui vit DANS `dest_dir` (une sauvegarde stockée
/// localement à côté de l'exécutable, courant pour les recomps/homebrews --
/// voir `resolve_save_folder`), la déplace vers `save_backup_dir` AVANT que
/// `dest_dir` ne soit supprimé -- sans ça, désinstaller un port effacerait
/// la sauvegarde avec le reste. Un save_folder EXTERNE à `dest_dir`
/// (ex: `%APPDATA%/...`, le cas le plus courant) n'est jamais dans la
/// trajectoire de suppression, donc jamais touché ici -- rien à faire pour
/// lui.
fn backup_local_save(save_folder: Option<&Value>, library_dir: &Path, dest_dir: &Path, folder_name: &str, field: &str) {
    let Some(save_folder) = save_folder else { return };
    let Some(save_path) = resolve_save_folder(save_folder, dest_dir) else { return };
    if !save_path.exists() || !save_path.starts_with(dest_dir) {
        return;
    }
    let backup_dir = save_backup_dir(library_dir, folder_name, field);
    let _ = fs::remove_dir_all(&backup_dir);
    if let Some(parent) = backup_dir.parent() {
        let _ = fs::create_dir_all(parent);
    }
    let _ = fs::rename(&save_path, &backup_dir);
}

/// Symétrique de `backup_local_save`, appelée après une extraction réussie
/// (voir `install_port`) -- remet en place une sauvegarde locale
/// précédemment préservée par une désinstallation, si `save_backup_dir`
/// contient encore quelque chose pour ce port/ce champ. Silencieux si rien
/// à restaurer (cas normal : la plupart des installs n'ont jamais eu de
/// sauvegarde locale sauvegardée).
fn restore_backed_up_save(save_folder: Option<&Value>, library_dir: &Path, dest_dir: &Path, folder_name: &str, field: &str) {
    let backup_dir = save_backup_dir(library_dir, folder_name, field);
    if !backup_dir.exists() {
        return;
    }
    let Some(save_folder) = save_folder else {
        let _ = fs::remove_dir_all(&backup_dir);
        return;
    };
    let Some(save_path) = resolve_save_folder(save_folder, dest_dir) else {
        let _ = fs::remove_dir_all(&backup_dir);
        return;
    };
    ...
    let _ = fs::rename(&backup_dir, &save_path);
}

/// `safe_join` fait remonter une erreur si `folder_name` sort de
...
pub fn uninstall_port(port: &Port, library_dir: &Path) -> Result<(), String> {
    let dest_dir = safe_join(library_dir, &port.folder_name)?;
    if dest_dir.exists() {
        backup_local_save(port.save_folder.as_ref(), library_dir, &dest_dir, &port.folder_name, "save_folder");
        backup_local_save(port.save_folder2.as_ref(), library_dir, &dest_dir, &port.folder_name, "save_folder2");
...
/// au démarrage) -- un `folder_name` invalide dans une seule entrée ne
...
pub fn is_installed(port: &Port, library_dir: &Path) -> bool {
    safe_join(library_dir, &port.folder_name).map(|p| p.exists()).unwrap_or(false)
}
...
mod tests {
    ...
    fn extract_exe_is_archive_ne_garde_que_le_dossier_de_l_exe() {
        ...
        extract(&archive_path, &dest_dir, &dir, true, None, &mut on_progress).unwrap();
    ...
    #[test]
    fn extract_exe_is_archive_echoue_si_plusieurs_dossiers_avec_exe() {
        ...
        let dest_dir = dir.join("dest");
        let mut on_progress: ProgressCallback = None;
        // Deux dossiers distincts contiennent chacun un exe -- jamais de
        // devinette au hasard entre les deux, l'install doit échouer
        // clairement plutôt que d'installer la mauvaise variante.
        assert!(extract(&archive_path, &dest_dir, &dir, true, None, &mut on_progress).is_err());
    }

    #[test]
    fn extract_exe_is_archive_base_exe_leve_l_ambiguite() {
        ...
        let dest_dir = dir.join("dest");
        let mut on_progress: ProgressCallback = None;
        extract(&archive_path, &dest_dir, &dir, true, Some("game.exe"), &mut on_progress).unwrap();
    ...
    fn extract_route_un_rar_vers_7z_plutot_que_de_le_copier_tel_quel() {
        ...
        let dest_dir = dir.join("dest");
        let mut on_progress: ProgressCallback = None;
        assert!(extract(&archive_path, &dest_dir, &dir, false, None, &mut on_progress).is_err());
        assert!(!dest_dir.join("release.rar").exists(), "aurait dû être extrait, pas copié tel quel");
    }

    #[test]
    fn extract_exe_is_archive_false_copie_l_exe_tel_quel() {
        ...
        let dest_dir = dir.join("dest");
        let mut on_progress: ProgressCallback = None;
        extract(&archive_path, &dest_dir, &dir, false, None, &mut on_progress).unwrap();
    ...
    fn port_with_save_folder(folder_name: &str, save_folder: Option<&str>) -> Port {
        port_with_save_folders(folder_name, save_folder, None)
    }
    ...
    fn port_with_save_folders(folder_name: &str, save_folder: Option<&str>, save_folder2: Option<&str>) -> Port {
        Port {
            ...
            mods_url: None,
            image_url: None,
            save_folder: save_folder.map(|s| Value::String(s.into())),
            save_folder2: save_folder2.map(|s| Value::String(s.into())),
            source_type: SourceType::DirectUrl,
            repo: None,
            exe_is_archive: false,
    ...
    fn uninstall_port_preserve_une_sauvegarde_locale() {
        ...
        let port = port_with_save_folder("MyGame", Some("Save"));
        ...
        uninstall_port(&port, &dir).unwrap();

        assert!(!dest_dir.exists());
        let backed_up = save_backup_dir(&dir, "MyGame", "save_folder").join("slot1.dat");
    ...
    fn uninstall_port_ne_touche_pas_une_sauvegarde_externe_au_dossier_du_jeu() {
        // save_folder absolu, en dehors de dest_dir (le cas le plus courant
        ...
        let port = port_with_save_folder("MyGame", Some(external_save.to_str().unwrap()));
        fs::create_dir_all(dir.join("MyGame")).unwrap();

        uninstall_port(&port, &dir).unwrap();

        assert!(external_save.join("slot1.dat").exists());
        assert!(!save_backup_dir(&dir, "MyGame", "save_folder").exists());
    }

    #[test]
    fn restore_backed_up_save_remet_la_sauvegarde_en_place() {
        let dir = temp_dir("restore_save");
        let port = port_with_save_folder("MyGame", Some("Save"));
        let backup = save_backup_dir(&dir, "MyGame", "save_folder");
        ...
        restore_backed_up_save(port.save_folder.as_ref(), &dir, &dest_dir, &port.folder_name, "save_folder");
    ...
    #[test]
    fn save_folder_et_save_folder2_ne_s_ecrasent_jamais_meme_avec_des_noms_de_fichier_identiques() {
        let dir = temp_dir("two_save_folders");
        let port = port_with_save_folders("MyGame", Some("Save"), Some("Save2"));
        let dest_dir = dir.join("MyGame");
        fs::create_dir_all(dest_dir.join("Save")).unwrap();
        ...
        let backup1 = save_backup_dir(&dir, "MyGame", "save_folder").join("slot1.dat");
        let backup2 = save_backup_dir(&dir, "MyGame", "save_folder2").join("slot1.dat");
        assert_eq!(fs::read_to_string(backup1).unwrap(), "from-save-folder");
        assert_eq!(fs::read_to_string(backup2).unwrap(), "from-save-folder2");
    }

    #[test]
    fn is_installed_faux_si_dossier_absent_et_ne_paniques_pas_sur_folder_name_invalide() {
    ...
    fn cycle_install_reinstall_uninstall_avec_archives_adversariales_ne_plante_jamais() {
        ...
        for cycle in 0..CYCLES {
            ...
            let port = port_with_save_folder(&format!("Game{cycle}"), Some("Save"));
            let dest_dir = library_dir.join(&port.folder_name);

            let mut on_progress: ProgressCallback = None;
            extract(&archive_path, &dest_dir, &library_dir, false, None, &mut on_progress).unwrap();
            ...
            let _ = uninstall_port(&port, &library_dir);
