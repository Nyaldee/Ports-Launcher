...
//!
//! Extraction : `extract_archive_via_7z` shelle vers un binaire 7-Zip
//! externe (`7z.exe` sous Windows, `7zzs` sous Linux), seule implémentation
//! pour zip/tar/tar.gz/7z/NSIS/RAR.
//!
...
pub enum InstallError {
    ...
    /// avec `AssetSelectionError`.
    Ambiguous(#[allow(dead_code)] String, Vec<Value>),
}

/// `github_api::GitHubError`/`gitlab_api::GitLabError` sont des alias de ce
/// même type (voir asset_select.rs) -- un seul `From` couvre les trois
/// call-sites de `install_port`.
...
/// Nom du binaire 7-Zip attendu pour cet OS -- voir `sevenzip_exe_path`.
/// Linux : `7zzs`, le build statique officiel -- aucune lib partagée
/// requise, cohérent avec le zéro-dépendance runtime du reste du projet.
#[cfg(target_os = "windows")]
const SEVENZIP_CANDIDATES: &[&str] = &["7z.exe"];
#[cfg(target_os = "linux")]
const SEVENZIP_CANDIDATES: &[&str] = &["7zzs"];

/// Chemin du binaire 7-Zip pour cet OS, attendu à côté de l'exécutable
/// (jamais empaqueté dans le binaire -- même convention que
/// `ports.json`/`themes.json`, voir `base_dir` dans main.rs, copié par
/// build.rs). Vérifie aussi un niveau au-dessus du dossier de l'exe :
/// `cargo test` tourne depuis target/<profil>/deps/, alors que build.rs
/// copie dans target/<profil>/, un niveau plus haut.
...
/// Repère la ligne `ERROR: ...` dans la sortie de 7-Zip (stderr d'abord,
...
/// Liste puis extrait `archive` via 7-Zip (`7z.exe` sous Windows, `7zzs`
/// sous Linux -- voir `sevenzip_exe_path`) -- couvre
/// zip/tar/tar.gz/7z/NSIS/RAR (y compris un `.exe` auto-extractible de
/// n'importe lequel de ces types) en une seule implémentation. Le type réel
/// est détecté à partir du listing technique (`Type = ...`), pas de
...
///
/// L'écriture des fichiers est déléguée à 7-Zip (processus externe) -- la
/// protection anti zip-slip est donc celle intégrée à 7-Zip lui-même.
