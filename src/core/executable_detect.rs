...
/// binaire) sont des candidats équivalents sous Windows.
#[cfg(target_os = "windows")]
fn is_candidate(path: &Path) -> bool {
    path.extension().is_some_and(|e| e.eq_ignore_ascii_case("exe") || e.eq_ignore_ascii_case("lnk") || e.eq_ignore_ascii_case("bat"))
}

/// Sous Linux, un port n'a ni extension `.exe` ni raccourci -- un candidat
/// est un fichier marqué exécutable (bit +x, ce que le noyau exige pour le
/// lancer directement), ou un script `.sh`/`.AppImage` sans ce bit encore
/// posé (fréquent après extraction d'une archive qui ne préservait pas les
/// permissions Unix -- voir `core::launch::launch`, qui sait relancer un
/// `.sh` via `sh` dans ce cas).
#[cfg(target_os = "linux")]
fn is_candidate(path: &Path) -> bool {
...
/// Descend récursivement dans `dir` et ajoute à `out` chaque FICHIER
/// candidat (voir `is_candidate`) dont le nom n'est pas un désinstalleur
...
/// Choisit l'exécutable du dossier du jeu quand un seul candidat est
/// trouvé (voir `is_candidate`/`collect_files_recursive`), sinon liste les
/// candidats pour un choix manuel.
///
...
mod tests {
    ...
    /// Crée un fichier candidat "de base" pour ce système -- `.exe` sous
    /// Windows, un fichier marqué exécutable (bit +x) sous Linux, chacun le
    /// strict minimum reconnu par `is_candidate`.
    #[cfg(target_os = "windows")]
    ...
    // Sous Windows, `.lnk` (raccourci) est un second type de candidat sans
    // équivalent direct en bit exécutable -- testé séparément de
    // write_candidate ci-dessus.
    #[cfg(target_os = "windows")]
    #[test]
    ...
    // Sous Linux, un `.sh` sans bit exécutable (archive qui ne préservait
    // pas les permissions Unix) est le second type de candidat -- voir
    // `is_candidate`.
    #[cfg(target_os = "linux")]
    #[test]
