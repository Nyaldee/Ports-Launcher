...
/// Cherche la taille de police (px) donnant une ligne d'environ
/// `target_linespace` de haut -- converge en quelques itérations, le
/// linespace croissant quasi linéairement avec la taille demandée. Retourne
/// `(taille_px, linespace mesuré à cette taille)` ; le second sert à
/// dimensionner les lignes/barres qui doivent contenir ce texte.
pub fn solve_font_for_height(family: &str, target_linespace: i32) -> (i32, i32) {
    let target = target_linespace.max(1);
    let mut size = (target as f32 * 0.75).round().max(8.0) as i32;
    let mut linespace = size;
    for _ in 0..4 {
        linespace = linespace_for_size(family, size);
        if linespace <= 0 || linespace == target {
            break;
        }
        size = ((size as i64 * target as i64) / linespace as i64).max(8) as i32;
    }
    (size, linespace)
}
