//! Mesures de texte (linespace, largeur, retour à la ligne). Il n'existe pas
//! de formule fermée entre une taille de police demandée et le linespace
//! réellement rendu (ça dépend de la police) : on mesure et on ajuste.
//!
//! Windows : GDI, faute d'API de mesure exposée par Slint au code hôte Rust,
//! alors que `windows` est déjà une dépendance.
//! Linux : la police système est résolue par `fontconfig` (`fc-match`, déjà
//! comment tout le reste de ce projet shelle vers un outil externe plutôt
//! que d'ajouter une dépendance de liaison -- voir `core::installer` et
//! `7z.exe`), puis mesurée via `fontdue` (glyphes/métriques, pur Rust).
...
pub fn wrapped_text_height(family: &str, px: i32, text: &str, max_width_px: i32) -> i32 {
    // `DrawTextW` avec `DT_WORDBREAK` et un tampon de longueur 0 (chaîne
    // vide) PLANTE (STATUS_ACCESS_VIOLATION, vérifié) plutôt que de renvoyer
    // une hauteur nulle -- jamais l'appeler pour ce cas, repli direct sur UNE
    // ligne (même valeur que linespace_for_size, cohérent avec la version
    // Linux ci-dessous, qui compte toujours au moins 1 ligne).
    if text.is_empty() {
    ...
    unsafe {
        with_font(family, px, |hdc| {
            ...
            // Une vraie mesure ratée (police/HDC en cause) donne aussi 0 ici
            // -- même repli sur une ligne que ci-dessus.
            if height > 0 {
                height
            } else {
...
/// Police système résolue par `fc-match` pour `family`, parsée par
/// `fontdue`, mise en cache (résolution + parsing du fichier ne sont
/// refaits qu'une fois par famille) -- `solve_font_for_height` appelle
/// `linespace_for_size` en boucle pour une même famille, le lookup/parsing
/// répété serait gratuit. `thread_local!` : Slint (comme le reste de
/// `AppState`) tourne entièrement sur le thread principal, pas besoin d'un
/// verrou.
#[cfg(target_os = "linux")]
...
/// Équivalent de la version Windows (`tmHeight`) -- `ascent - descent +
/// line_gap` est l'équivalent standard du linespace en typographie
/// numérique (OpenType `hhea`), ce que `fontdue` expose directement.
#[cfg(target_os = "linux")]
pub fn linespace_for_size(family: &str, px: i32) -> i32 {
...
/// Somme des largeurs d'avance des glyphes -- pas de kerning/ligatures
/// (`fontdue` ne shape pas le texte), suffisant pour dimensionner une mise
/// en page, contrairement à un rendu pixel-perfect qui en aurait besoin.
#[cfg(target_os = "linux")]
...
/// Équivalent de la version Windows.
#[cfg(target_os = "linux")]
pub fn max_text_width<'a>(family: &str, px: i32, texts: impl Iterator<Item = &'a str>) -> i32 {
...
/// Équivalent de la version Windows -- repli à la main sur un algorithme de
/// retour à la ligne glouton (mot par mot, jamais coupé), faute d'un
/// `DT_CALCRECT`/`DT_WORDBREAK` natif hors GDI.
#[cfg(target_os = "linux")]
pub fn wrapped_text_height(family: &str, px: i32, text: &str, max_width_px: i32) -> i32 {
    let Some(font) = cached_font(family) else { return px };
...
/// nombre de mesures, mais seule la marche qui suit (jamais bornée à un
...
/// Largeur du plus long segment sans espace de `text`. Un retour à la ligne
/// mot par mot ne coupe jamais À L'INTÉRIEUR d'un mot, seulement aux
/// espaces : un nom de fichier sans espace déborde donc du cadre quelle que
/// soit la largeur disponible, même avec une hauteur de dialogue
/// correctement mesurée. Sert à élargir le dialogue à ce mot AVANT de
/// mesurer le repli (voir dialog_geometry.rs), plutôt qu'à couper le mot.
...
mod tests {
    ...
    /// Texte vide : les deux plateformes doivent compter UNE ligne (voir
    /// leur commentaire respectif), pas 0 -- un statut/message vide ne doit
    /// jamais rendre un dialogue plus bas que sa police ne l'exige.
    #[test]
