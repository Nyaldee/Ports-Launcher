...
/// le bureau non-EWMH. Sert uniquement à la géométrie INITIALE de la
/// fenêtre (voir main.rs) : winit/Slint replacent de toute façon la fenêtre
/// sur le bon moniteur une fois réellement mappée.
pub fn work_area_under_cursor() -> (i32, i32, i32, i32) {
    let Ok((conn, screen_num)) = x11rb::connect(None) else { return (0, 0, 1920, 1080) };
    let screen = &conn.setup().roots[screen_num];
    let fallback = (0, 0, screen.width_in_pixels as i32, screen.height_in_pixels as i32);
    (|| -> Result<(i32, i32, i32, i32), Box<dyn std::error::Error>> {
        let atom = intern(&conn, "_NET_WORKAREA")?;
        let reply = conn.get_property(false, screen.root, atom, AtomEnum::CARDINAL, 0, 4)?.reply()?;
        let mut values = reply.value32().ok_or("_NET_WORKAREA absente ou mal typée")?;
        // Toujours la zone du 1er bureau virtuel -- un futur repli sur
        // `_NET_CURRENT_DESKTOP` n'aurait d'intérêt qu'avec plusieurs bureaux
        // ayant CHACUN un dock/panel différent, cas non rencontré en pratique.
        let (x, y, w, h) = (values.next(), values.next(), values.next(), values.next());
        match (x, y, w, h) {
            (Some(x), Some(y), Some(w), Some(h)) if w > 0 && h > 0 => Ok((x as i32, y as i32, w as i32, h as i32)),
            _ => Err("_NET_WORKAREA incomplète".into()),
        }
    })()
    .unwrap_or(fallback)
}
