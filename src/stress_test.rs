...
//! `use super::*;` lui donne accès à tout ce que main.rs importe lui-même
//! (AppState, GamepadRouter, base_dir, PAGE_ROWS, l'import `slint`...) ; le
//! reste (fonctions d'orchestration vivant dans `app::*` mais que main.rs
//! n'appelle pas directement) est importé explicitement ci-dessous.
use super::*;
...
/// plutôt qu'un thème de repli. `state.json` part avec
/// `last_launcher_update_check` à "maintenant", ce qui désactive
/// `should_check_launcher_update()` : `start_self_update_check` n'atteint
/// jamais le réseau pendant ce run (les vérifications par port, elles,
/// n'ont de toute façon rien à vérifier tant qu'aucun port n'est installé --
/// voir `"installed": {}` plus bas).
///
