...
pub(crate) struct GridNav {
    ...
    /// `chrome::double_click_time_ms()`, lu une fois au démarrage -- le
    /// délai réel configuré par l'OS sous Windows, une valeur fixe sous
    /// Linux (voir linux_chrome), jamais une valeur en dur qui ignorerait
    /// les réglages d'accessibilité de l'utilisateur quand ils existent.
...
impl AppState {
    ...
    /// Jetons GitHub/GitLab courants, à cloner AVANT de passer la frontière
    /// vers un thread d'arrière-plan (voir `start_install`/`open_version_picker`/
    /// `launch_with_update_check`) -- `RefCell<StateManager>` n'est de toute
    /// façon pas `Send`.
