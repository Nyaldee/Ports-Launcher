    ...
    for (i, event) in drained.into_iter().enumerate() {
        match event {
            ...
            AppEvent::PlayUpdateChecked { port, available } => {
                // Libère le slot réservé par launch_with_update_check --
                // AVANT start_install, qui le réserve lui-même et abandonnerait
                // en silence en le trouvant déjà pris.
...
fn reclaim_dialog_foreground_if_needed(app: &Rc<AppState>) {
    let dialog_native = super::dialogs::dialog_window(&app.dialog_nav.dialogs.borrow()).and_then(chrome::native_window);
    let (Some(dialog_native), Some(main_native)) = (dialog_native, chrome::native_window(app.window().window())) else { return };
    if chrome::is_foreground_window(main_native) {
        chrome::force_foreground_window(dialog_native);
    }
}
