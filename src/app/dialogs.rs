...
pub(crate) fn close_current_dialog(app: &Rc<AppState>, router: &Rc<RefCell<GamepadRouter>>) {
    ...
    if had_dialog {
        ...
        if let Some(native) = chrome::native_window(app.window().window()) {
            chrome::set_window_enabled(native, true);
        }
...
/// véritablement modale (voir chrome::set_window_enabled, réactivée dans
...
pub(crate) fn finish_dialog_open(app: &Rc<AppState>, router: &Rc<RefCell<GamepadRouter>>, slot: DialogSlot) {
    ...
    if let Some(native) = chrome::native_window(app.window().window()) {
        chrome::set_window_enabled(native, false);
    }
    *app.dialog_nav.dialogs.borrow_mut() = slot;
    push_dialog_target(app, router);

    // DIFFÉRÉ : `chrome::native_window()` renvoie None juste après l'ouverture du
    ...
    slint::Timer::single_shot(std::time::Duration::from_millis(50), move || {
        ...
        let Some(native) = dialog_window(&slot_ref).and_then(chrome::native_window) else { return };
        drop(slot_ref);
...
/// chrome::set_window_enabled) devrait déjà empêcher tout callback de la
