...
///
/// Réserve le slot d'opération (voir `try_claim_op_slot`) comme les trois
/// autres chemins réseau de ce fichier -- sans ça, un second clic sur Play
/// pendant que le check tourne encore repasserait la garde de `activate_port`
/// (qui ne voit rien dans `installing`) et déclencherait un second appel
/// GitHub/GitLab redondant pour le même port. Libéré par le handler
/// d'`AppEvent::PlayUpdateChecked`.
