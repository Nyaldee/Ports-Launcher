...
parser.add_argument('--force', action='store_true', help='Ignore ROM hash checks (use if your ROM is a valid but unrecognized variant, e.g. an updated hack)')
parser.add_argument('--assume-language', action='store', metavar='LANGCODE', default='us', help='Language to assume when --force is used and the ROM hash is unrecognized (default: us)')
...
if args.extract_dialogue:
  ROM = util.load_rom(args.rom, True, args.force, args.assume_language)
  import extract_resources, sprite_sheets
  sprite_sheets.decode_font()
  extract_resources.print_dialogue()
  sys.exit(0)

ROM = util.load_rom(args.rom, ignore_unknown_hash = args.force, fallback_language = args.assume_language)
